...
#define STATIC_SHM_SIZE (2 * 4096 - WORKLOAD_START)
...
int u_socket (int domain, int type, int protocol)
{
    ...
    struct channel_cmd *cmd = (struct channel_cmd *)lwp_shminfo(shmid);
    if (cmd)
    {
        fd = (int)ipc_cmd_send_recv(usocket_channel, (void*)(size_t)shmid, 0);
        lwp_shmdt(cmd);
        lwp_shmrm(shmid);
    }
...
int u_bind (int socket, const struct sockaddr *name, socklen_t namelen)
{
    ...
    if (cmd)
    {
        void *ptr = (void*)cmd + WORKLOAD_START;
        memcpy(ptr, name, namelen);
        res = (int)ipc_cmd_send_recv(usocket_channel, (void*)(size_t)shmid, 0);
        lwp_shmdt(cmd);
    }
