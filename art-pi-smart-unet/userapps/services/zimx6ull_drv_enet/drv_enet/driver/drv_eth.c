...
rt_err_t enet_buffer_init(enet_buffer_config_t *buffConfig)
{
    ...
    tx_bd_addr = rt_malloc(SYS_PAGE_SIZE<<TX_BUFFER_INDEX_NUM);
    if(!tx_bd_addr)
    {
    ...
    buffConfig->txBdStartAddrAlign = (void *)ueth_remap(ueth_v2p(tx_bd_addr), UETH_REMAP_NOCACHE, (SYS_PAGE_SIZE<<TX_BUFFER_INDEX_NUM));
...
// static rt_err_t rt_imx6ul_eth_init_empty(rt_device_t dev)
// {
//     return 0;
// }
...
int imx6ul_eth_init(int eth_select, struct rt_imx6ul_ethps** ueth)
{
...
// #endif        
    ...
    {
        *ueth = &_imx6ul_eth_device[eth_select];
    }
