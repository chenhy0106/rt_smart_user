...
static void iperf_udp_client(void *thread_param)
{
    ...
    sock = u_socket(PF_INET, SOCK_DGRAM, 0);
    if(sock < 0)
    {
    ...
    while (param.mode != IPERF_MODE_STOP)
    {
        ...
        u_sendto(sock, buffer, send_size, 0, (struct sockaddr *)&server, sizeof(struct sockaddr_in));
    }
    closesocket(sock);
    rt_free(buffer);
}
...
static void iperf_udp_server(void *thread_param)
{
    ...
    timeout.tv_sec = 2;
    timeout.tv_usec = 0;
    if (u_setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout)) == -1)
    {
    ...
    if (u_bind(sock, (struct sockaddr *)&server, sizeof(struct sockaddr_in)) < 0)
    {
    ...
    while (param.mode != IPERF_MODE_STOP)
    {
        ...
        {
            r_size = u_recvfrom(sock, buffer, IPERF_BUFSZ, 0, (struct sockaddr *)&sender, (socklen_t*)&sender_len);
            if (r_size > 12)
            {
...
static void iperf_client(void *thread_param)
{
    ...
    while (param.mode != IPERF_MODE_STOP)
    {
        sock = u_socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
        if (sock < 0)
        {
        ...
        while (param.mode != IPERF_MODE_STOP)
        {
            ...
            ret = u_send(sock, send_buf, IPERF_BUFSZ, 0);
            if (ret > 0)
            {
                sentlen += ret;
            }
...
void iperf_server(void *thread_param)
{
    ...
    sock = u_socket(AF_INET, SOCK_STREAM, 0);
    if (sock < 0)
    {
        LOG_E("Socket error!");
        goto __exit;
    ...
    if (u_bind(sock, (struct sockaddr *)&server_addr, sizeof(struct sockaddr)) < 0)
    {
        LOG_E("Unable to bind!");
        goto __exit;
    }

    if (u_listen(sock, 5) < 0)
    {
        LOG_E("Listen error!");
        goto __exit;
    }
